  ...
  <div class="doc-contributors">
    ...
    <div class="contributors-list">
      ...
      >
        ...
        />
        <div v-if="showTooltip === contributor.id" class="contributor-tooltip">
          {{ contributor.login }}
        </div>
      </div>
      ...
      <div v-if="loading" class="loading-contributors">
        <div class="loading-text">正在加载贡献者信息...</div>
      </div>

      <!-- 如果没有找到贡献者，显示默认信息 -->
      <div v-else-if="contributorsList.length === 0" class="no-contributors">
        <div class="no-contributors-text">暂无贡献者信息</div>
      </div>
    </div>
  </div>
</template>
...
const contributorsList = computed(() => {
  ...
  if (!props.componentName) {
    return (
      allContributorsData.value.contributors?.slice(0, props.maxCount) || []
    );
  }
  ...
  if (allContributorsData.value.componentContributors) {
    ...
    const matchingComponents =
      allContributorsData.value.componentContributors.filter((comp: any) => {
  ...
  if (allContributorsData.value.fileContributors) {
    const componentFiles = allContributorsData.value.fileContributors.filter(
      (fc: FileContributor) => {
        const fileName = fc.file.toLowerCase();
        const componentName = props.componentName.toLowerCase();

        return (
          fileName.includes(`src/${componentName}/`) ||
...
const loadData = async () => {
  try {
    ...
    console.log(
      `Contributors data loaded for component: ${props.componentName}`,
      data,
    );
  } catch (error) {
