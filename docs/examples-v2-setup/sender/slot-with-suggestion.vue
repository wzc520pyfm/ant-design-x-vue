...
defineOptions({ name: 'AXSenderSlotWithSuggestionV2Setup' });
...
import { Attachments, Sender, SenderHeader, SenderSwitch, Suggestion, type SenderProps, type AttachmentsProps } from 'ant-design-x-vue';
  ...
  senderRef.value?.insert?.([
    {
      ...
      props: {
        label: FileInfo[item.key as string].label,
        value: item.key,
      },
    },
  ]);
...
<template>
  <a-flex vertical gap="middle">
    <Suggestion :items="suggestions" @select="onSuggestionSelect">
      <template #default="{ onTrigger }">
        <Sender
          ...
          @submit="onSubmit"
          @cancel="onCancel"
          @keydown="(e: KeyboardEvent) => { e.key === '@' ? onTrigger() : onTrigger(false); }"
        >
          <template #header>
            <SenderHeader
              title="Attachments"
          ...
          <template #footer="{ ori }">
            <a-flex justify="space-between" align="center">
              <a-flex gap="small" align="center">
                ...
                <a-dropdown v-if="fileItems?.length" :menu="{ onClick: fileItemClick, items: fileItems }">
                  <SenderSwitch :value="false">
              ...
              <a-flex align="center">
                <a-button type="text" :style="{ fontSize: '16px' }">
